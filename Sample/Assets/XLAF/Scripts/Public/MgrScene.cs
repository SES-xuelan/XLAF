...
namespace XLAF.Public
{
    ...
    public class MgrScene : MonoBehaviour
    {
        ...
        /// <summary>
        /// Diy Usage:
        /// var param = new XLAF.Private.SceneParams();
        /// param.sceneName = "LevelTitle";
        /// param.data = 1;
        /// param.anim = SceneAnimation.diy;
        /// param.diyOldSceneAnimAction = (sceneObj, complete) =>
        /// {
        ///     iTween.ValueTo(sceneObj.scene, new iTweenHashBuilder()
        ///         .SetFrom(1f)
        ///         .SetTo(0f)
        ///         .SetTime(1)
        ///         .SetOnUpdate((float f) =>
        ///         {
        ///             sceneObj.ChangeAlpha(f);
        ///             sceneObj.scene.transform.localRotation = Quaternion.Euler(0, 0, f * 360f);
        ///             sceneObj.scene.transform.localScale = new Vector3(f, f, f);
        ///         })
        ///         .SetOnComplete(complete)
        ///         .Build());
        /// };
        /// param.diyNewSceneAnimAction = (sceneObj, complete) =>
        /// {
        ///     iTween.ValueTo(sceneObj.scene, new iTweenHashBuilder()
        ///         .SetFrom(0f)
        ///         .SetTo(1f)
        ...
        public static void GotoScene(SceneParams par)
        {
            ...
            var diyOldAction = par.diyOldSceneAnimAction;
            var diyNewAction = par.diyNewSceneAnimAction;
            ...
            switch (anim)
            {
                ...
                case SceneAnimation.zoomOut:
                    ...
                    break;
                case SceneAnimation.diy:
                    instance._AnimationDiy(sceneName, data, diyOldAction, diyNewAction, cb);
                    break;
            }
        ...
        private void _AnimationDiy(string sceneName, object data, Action<SceneObject, Action> oldSceneAnim, Action<SceneObject, Action> newSceneAnim, Action cb)
        {
            if (oldSceneAnim == null || newSceneAnim == null)
            {
                _AnimationNone(sceneName, data, cb);
                return;
            }
            ...
            if (oldScene != null)
            {
                ...
                //reset alpha to 1
                oldScene.ChangeAlpha(1f);
                oldSceneAnim(oldScene, () =>
                {
                    oldScene.script.ExitScene();
                    _UnloadOldScene(oldScene);
                    _LoadNewScene(sceneName, data);
                    newSceneAnim(currentScene, () =>
                    {
                        if (cb != null)
                            cb();
                        currentScene.script.EnterScene(data);
            ...
            {
                // load new scene  or   set exise scene
                _LoadNewScene(sceneName, data);
                //show
